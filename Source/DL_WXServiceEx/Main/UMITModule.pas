...
begin
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if UpperCase(nStr) = UpperCase(sFlag_WXFactory) then
          gSysParam.FFactID := Fields[0].AsString;
        //fact id

        if UpperCase(nStr) = UpperCase(sFlag_WXSrvRemote) then
          gSysParam.FSrvRemote := Fields[0].AsString;
        //local mit

        if UpperCase(nStr) = UpperCase(sFlag_WXServiceMIT) then
          gSysParam.FSrvMIT := Fields[0].AsString;
        //local mit

        if UpperCase(nStr) = UpperCase(sFlag_Rq_WXUrl) then
          gSysParam.FSrvUrl := Fields[0].AsString;
        //请求微信Url

        if UpperCase(nStr) = UpperCase(sFlag_Rq_WXPicUrl) then
  ...
  begin
    nStr := '未配置[ %s.%s ]字典项.';
    nStr := Format(nStr, [sTable_SysDict, gSysParam.FFactID]);
